...
class AffineLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        Returns:
            outputs: Array of layer outputs of shape (batch_size, output_dim).
        """
        xw_t = np.dot(inputs, np.transpose(self.weights))
        return xw_t + self.biases
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        grads_wrt_weights = np.dot(np.transpose(inputs), grads_wrt_outputs)
        return [np.transpose(grads_wrt_weights), grads_wrt_outputs.sum(axis=0)]
